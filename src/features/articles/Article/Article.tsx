...
export const Article = ({ article, layout }: PropTypes) => {
  ...
  return (
    <article
      ...
      data-layout={layout}
    >
      {article?.__typename !== 'VideoRecord' ? (
        <>
            ...
            <StructuredText
              renderBlock={({ record }) => {
                if (record.__typename === 'ImageRecord') {
                  ...
                  record.__typename === 'ExternalVideoRecord'
                ) {
                  return (
                  ...
                  record.__typename === 'InternalVideoRecord'
                ) {
                  return (
                    <div className={styles.videoContainer}>
        ...
        <>
          <h1 className={styles.title}>{article?.description}</h1>
          <span className={styles.date}>{formattedDate}</span>
          <div className={styles.body}>
            <div className={styles.videoContainer}>
              <iframe
                src={getVideoEmbedUrl(article?.file?.url)}
              ></iframe>
            </div>
          </div>
        </>
